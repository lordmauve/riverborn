...
class HeightfieldApp(mglw.WindowConfig):
    ...
    def create_grid(self):
        ...
        self.vertices = np.zeros(num_vertices, dtype=[
        ...
        for i, z in enumerate(zs):
            for j, x in enumerate(xs):
                n = noise.pnoise2(x * self.noise_scale, z * self.noise_scale,
        ...
        vertex_normals = np.zeros((self.segments+1, self.segments+1, 3), dtype=np.float32)
    ...
    def create_texture(self):
        ...
        self.texture = self.ctx.texture((tex_width, tex_height), 3, texture_data.tobytes())
